...
def _generate_accurip_for_disc(ffmpeg_exe, album_dir, disc_num, track_paths, config):
    ...
    lines.append(f"Generated by Music Library Optimizer")
    lines.append("")
    # Per-track
    for idx, path in enumerate(track_paths):
        ...
        # Try to get track number for display
        tn = _tn(path)
        # Compute AR CRC
        ar_crc = _accuraterip_crc(ffmpeg_exe, path, is_first_track=is_first, is_last_track=is_last)
        # Also get EAC CRC for reference (from log if available, else compute)
        # For now, just show computed AR CRC
        if ar_crc:
            lines.append(f"Track {tn:02d}: [{ar_crc}]  {base}")
        else:
            lines.append(f"Track {tn:02d}: [--------]  {base}  (could not compute)")
    lines.append("")
