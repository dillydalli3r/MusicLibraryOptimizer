# Music Library Optimizer v1.7.0
...
**Desktop app (installer):** Download `MusicLibraryOptimizer_Setup_v1.7.0_x64.exe`
...
**Portable:** Download `MusicLibraryOptimizer_v1.7.0_portable_x64.exe` and run it
...
- **AccurateRip `.accurip` files like CUETools for CD rips** — new `mlo/accurip.py` `run_generate_accurip` (script `9 Generate AccurateRip`, `Settings → AccurateRip 06b` `write_accurip_files`/`force_accurip`, `run_all_order` now `9` scripts) computes `AccurateRip CRC` per `D-TT` track via `ffmpeg s16le` → `AR` sum (offset handling) and writes canonical `CD-1.accurip`/`CD-2.accurip` (`AccurateRip verification for … — Generated by Music Library Optimizer` + `Track 01: [XXXXXXXX]  1-01 Test.flac`) with `mlo/accurip.py:_canonical_accurip_text` (trim each line `strip(" \t")`, `collapse` blanks, no leading/trailing blanks). `MEDIA=CD` `D-TT` or single-disc fallback, `ffmpeg` required, `force` respects `ENCODER` skip.
