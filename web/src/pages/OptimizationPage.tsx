...
import {
  Check, ChevronDown, Gauge, Play, RefreshCw, Wand2, Zap,
} from "lucide-react";
import { api } from "../api";
  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      <p className="text-xs text-zinc-500 mt-1">
        Run the library maintenance scripts — individually or all together. Force re-runs scripts
        that would normally skip because they are already done.
      </p>

      {/* ---- Run All + Force ------------------------------------------ */}
      <div className="bg-card rounded-lg border border-border p-4 mt-5">
        <div className="flex items-center gap-2 flex-wrap">
          <button className="btn-primary text-xs" disabled={busy} onClick={runAll}>
      ...
      <div className="bg-card rounded-lg border border-border p-4 mt-4">
        ...
        <div className="grid sm:grid-cols-2 gap-1.5">
          {SCRIPTS.map((s) => (
            <button
              key={s.label}
              className="btn-ghost !py-1.5 text-xs justify-start"
              disabled={busy}
        ...
        <div className="text-[10px] text-zinc-600 mt-2">
          Tip: to run scripts on a specific album or track selection, use the selection menu in the
          library view — those runs only touch what you selected.
        </div>
      </div>
    </div>
  );
